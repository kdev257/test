...
from .models import Purchase_Order,Material_Receipt_Note,Quality_Check,Gate_Entry,Vehicle_Unloading_Report,Freight_Purchase_Order,Vehicle_Unload_Items,Mrn_Items
    ...
    if request.method == 'POST':
        try:
            with db_transaction.atomic():
                form = QuotationItemsForm(request.POST)
...
def po_entry(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            # try:
                with db_transaction.atomic():  # Start an atomic transaction
                    instance = form.save(commit=False)
                    instance.unit = request.user.user_roles.unit
                    instance.created_by = get_object_or_404(User, id= request.user.id)
                    ...
                    # Attempt to redirect
                    return redirect('purchase_order_items',id)
            # except Exception as e:
            #     # Log the error if necessary
            #     print(f"Redirection failed: {e}")
            #     # The transaction will be rolled back automatically
            #     form.add_error(None, "There was an error processing the request. Please try again.")
    else:
            ...
            try:
                with db_transaction.atomic():
                    ...
                    for form,it in zip(formset,items):                        
                        po_quantity = form.cleaned_data['po_quantity']                        
                        it.balance_quantity -= po_quantity
                        it.quotation.balance_quantity -= po_quantity
                        it.save() 
                        it.quotation.save()
                        print(it.balance_quantity,it.quotation.balance_quantity) 
                        purchase_order = get_object_or_404(Purchase_Order,quotation=it.quotation.id )
                        purchase_order.po_quantity += Decimal(po_quantity)
                        rate = it.unit_rate
                        value = po_quantity * rate
                        purchase_order.po_value += value
                        conversion_rate = it.currency.conversion_rate
                        ...
                        print(quotation_item,209)                    
                        tax = calculate_tax(po=purchase_order,value=inr_value,supplier_state=state,item=it.item)                        
                        vat,cst,cgst,sgst,igst = tax['VAT'],tax['CST'],tax['CGST'],tax['SGST'],tax['IGST']
                        creditable= tax['creditable']
                        tax = vat+cst+cgst+sgst+igst
                        purchase_order.tax += tax                     
                        purchase_order.save()
                        instance =Purchase_Order_Items(purchase_order=purchase_order,quantity= po_quantity,rate=rate,value=value,inr_value=inr_value,quotation_item=quotation_item,balance_quantity=balance_quantity,vat=vat,cst=cst,cgst=cgst,sgst=sgst,igst=igst,creditable=creditable)
                        instance.save()
...
def freight_purchase_order(request,id):    
    po = Purchase_Order.objects.get(id=id) 
    transaction_type = Transaction_Type.objects.get(transaction_name='FPO')
    ...
    lcr_service = po.freight.service.service.id    
    lcr_transporter = po.freight.service.supplier
    currency = lcr_transporter.currency
    inr_amount = lcr_freight * currency.conversion_rate
    ...
    freight_po = Freight_Purchase_Order.objects.create(transaction_type=transaction_type,po_amount=lcr_freight,cgst=cgst,sgst=sgst,igst=igst,business= business,created_by=created_by,service=Service.objects.get(id=lcr_service),supplier=lcr_transporter,unit=po.unit,po=po,inr_amount=inr_amount)
    freight_po.save()    
...
def gate_entry_awaiting_unloading(request):
    ...
    if query:        
        ...
        register = Gate_Entry.objects.filter(is_unloaded=False)
...
def quality_check(request, id):
    ...
    print(gate_entry_instance.item_cat)
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            try:
                ...
                gate_entry_instance.is_quality_ok=True
...
def vehicle_unload(request,id):    
    ...
    if request.method =='POST':
        ...
        if form.is_valid():
            ...
            gate_entry_instance.save()
            id = instance.id
            messages.success(request,'Vehicle unloading report generate Successfully')           
...
def vehicle_unload_items(request,id):
    ...
    if request.method =="POST":
        ...
        if form.is_valid() and decision_form.is_valid():
            ...
            instance.save()
            unloading_instance.gate_entry.is_unloaded= True            
            response = decision_form.cleaned_data['add_item']
            if response == 'yes':
                return redirect('vehicle_unload_items', id)
            else:
        ...
        if form.is_valid():
            try:
                with db_transaction.atomic():
                    ...
                    id = last_mrn.id 
                    if gate_entry.po.business.pk ==2:                
                        process_mrn(id)
                    else:
...
def process_mrn(id):    
    ...
    if Inv_Transaction.transaction_number:
        last_transaction = Inv_Transaction.objects.select_for_update().filter(unit=unit,transaction_type=transaction_type).order_by('-id').first()
        ...
        new_transaction_no = f'{transaction_type}/{unit}/{str(new_number).zfill(5)}'
    else:
        new_transaction_no = f'{transaction_type}/{unit}/000001'   
    with db_transaction.atomic():
    ...
    #             # Fetch all custom duties related to the item
            ...
            if mrn_item.item.item_tax_type == 'Non_GST':
                excise_levies = State_Excise_taxes_On_Goods.objects.get(state=po.unit.state)       
                excise_name = excise_levies.tax_name       
                excise_rate = excise_levies.rate
                excise_uom = excise_levies.levy_unit
                excise_amount = billed_quantity * excise_rate
                total_excise_levies += excise_amount
                excise_levies = total_excise_levies
    ...
    #                 # Calculate applicable freight
            ...
            if po.business.pk == 2:
                landed_cost=0                                             
                landed_cost += value + cst +freight + cgst_freight + sgst_freight + igst_freight + excise_levies+custom_duty + igst + cgst + sgst                             
                credit_supplier += value + cst + vat + cgst + sgst + igst
                freight_provision += freight + cgst_freight + sgst_freight + igst_freight
                credit_custom += custom_duty
                credit_excise += excise_levies            
        ...
        if excise_levies >0:
            ...
            excise_update.save()
            update_account_chart(id=account_chart.id,credit_amount=credit_excise,debit_amount=0)
        ...
        if vat > 0:
            if creditable:
                ...
                vat_update.save()
                update_account_chart(id=account_chart.id,debit_amount=mrn.vat,credit_amount=0)
                total_debits += vat
